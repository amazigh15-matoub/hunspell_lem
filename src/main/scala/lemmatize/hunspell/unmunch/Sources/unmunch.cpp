...
int main(int argc, char** argv) {
  ...
  while (fgets(ts, MAX_LN_LEN - 1, wrdlst)) {
    ...
    for (i = 0; i < numwords; i++) {
		...
		// stripping of flags
		char *stmp = mystrdup(wlist[i].word);
		char *ptr;
		if (ptr = strchr(stmp,'/')) *ptr = 0;
		if (strlen(stmp) > 1) if ((stmp[strlen(stmp)-1]) == '0') stmp[strlen(stmp)-1] = 0;
		if ((ptr = strchr(stmp,'\'')) == NULL) ptr = stmp;
...
int expand_rootword(const char* ts, int wl, const char* ap) {
  ...
  // unit prefixes are not processed
  if (*ap == 'U') return (0);
